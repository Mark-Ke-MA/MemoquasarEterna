...
- `ensure_config` 对接 OpenClaw adapter 本地配置引导
...
以 `Adapters/hermes/CONNECTOR.py` 为例，当前暴露：
...
这说明当前 Hermes adapter 是 experimental partial implementation。

其中：

- `ensure_config` 对接 Hermes adapter 本地配置引导
- `production_agent.extract` 对接 Hermes `state.db` 到 Layer0 L2 的提取
- `production_agent.prerequisites` / `install` / `uninstall` 对接 Hermes profile 下的 Layer4 recall skill 安装生命周期
- `memory_worker.*` 当前未实现，因此不应把 `memory_worker_harness` 设置为 `hermes`
- `production_agent.preserve`、`production_agent.decay` 当前未实现
