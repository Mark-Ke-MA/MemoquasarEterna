...
def run_prerequisites(*, repo_root: str | Path | None = None, dry_run: bool = False) -> dict[str, Any]:
    ...
    preflight = require_openclaw_harness(config, action='memory worker prerequisites')
    ...
    config_updated = bool(root_check.get('config_updated'))
    if config_updated:
...
def main() -> None:
    parser = argparse.ArgumentParser(description='OpenClaw memory worker prerequisites check.')
